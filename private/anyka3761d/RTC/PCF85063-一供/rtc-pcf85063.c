...
static int pcf85063_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	 ...
	 */
	 /* 修改RTC的电容为12.5pf，切控制其它位为0，防止默认寄存器值存在异常 */
	ctrl1 = 0x01;
				...
				const struct i2c_device_id *id)
{
	...
	err = i2c_smbus_read_i2c_block_data(client, PCF85063_REG_TYPE,
					   sizeof(regs), regs);
	dev_err(&client->dev, "%s: read PCF85063_REG_TYPE vlaue = 0x%02x\n", __func__, regs[0]);

	if (regs[0] != 0x18) {
		dev_err(&client->dev, "RTC chip is not pcf85063 !\n");
		return -ENODEV;
	}
